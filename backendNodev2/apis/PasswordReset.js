...
const pwdgenerator = ()=> {
    const text = 'qwertyuiopasdfghjklzxcvbnmASDFGHJKLPOIUYTREWQZXCVBNM0125896347@#$'
    var pwd = ''
    for(let i = 0; i<=7; i++){
...
const reset = async (username, pwd) =>{
    try {
        ...
        const update = await User.updateOne({'username':username},{'$set':{'password':newPassword}})
...
exports.receive= (req,res)=>{
    try {
        res.send(`im recieving ${req.params.username , pwdgenerator()}`)
    } catch (error) {
        console.log(error)
    }
}
...
exports.changePassword = async (req,res) => {
    try {
        ...
        if(await verifypwd(oldPassword,details.password)){
            const update = await User.updateOne({'username':username},{'$set':{'password':newpwd}})
            if(update.modifiedCount === 1){
                res.json({
